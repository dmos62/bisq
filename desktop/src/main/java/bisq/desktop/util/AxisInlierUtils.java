...
public class AxisInlierUtils {
    ...
    /* Returns a ListChangeListener that is meant to be attached to an ObservableList.
    ...
    private static void applyRange(NumberAxis axis, int maxNumberOfTicks, Tuple2<Double, Double> bounds) {
        ...
        //axis.lowerBoundProperty().set(lowerBound);
        //axis.upperBoundProperty().set(upperBound);
        ...
        //axis.tickUnitProperty().set(tickUnit);
    }
    ...
    /* Extracted from cern.extjfx.chart.DefaultTickUnitSupplier.
     * Original description; note that the `multipliers` vector is hardcoded in the method to the default value used
     * in the source class:
     *
